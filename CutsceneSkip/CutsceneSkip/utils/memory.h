...
namespace shared
{
    struct memory_t
    {
        ...
        void attach( uint32_t pid )
        {
            if ( handle )
            {
                LI_FN( printf )( xorstr_( "[+] process aldready attached\n" ) );
                return;
            }
            ...
            if ( handle )
            {
                LI_FN( printf )( xorstr_( "[+] process attached\n" ) );
            }
        }
        ...
        auto get_module_handle( HANDLE p_handle, const char* name )
        {
            HMODULE null = { nullptr };
            HMODULE mods[ 1024 ];
            DWORD cb_needed;

            if ( LI_FN( EnumProcessModules )( p_handle, mods, sizeof( mods ), &cb_needed ) )
            {
                for ( auto i = 0; i < ( cb_needed / sizeof( HMODULE ) ); i++ )
                {
                    TCHAR szModName[ MAX_PATH ];
                    if ( LI_FN( GetModuleBaseNameA )( p_handle, mods[ i ], szModName, sizeof( szModName ) / sizeof( TCHAR ) ) )
                    {
        ...
        auto get_module_info( HANDLE p_handle, const char* name )
        {
            HMODULE module_handle = get_module_handle( p_handle, name );
            MODULEINFO info;
            LI_FN( GetModuleInformation )( p_handle, module_handle, &info, sizeof( info ) );
            return info;
        }

        std::uintptr_t find_pattern( HANDLE p_handle, const char* mod, const char* pattern )
        {
            auto mod_info = get_module_info( p_handle, mod );
            ...
            const auto image_bytes = static_cast<unsigned char*>( LI_FN( VirtualAlloc )( nullptr, image_size, MEM_COMMIT, PAGE_EXECUTE_READWRITE ) );
            LI_FN( ReadProcessMemory )( p_handle, reinterpret_cast<void*>( base_addr ), image_bytes, image_size, nullptr );
