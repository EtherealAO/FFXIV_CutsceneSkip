...
extern "C" __declspec(dllexport) int initialize( int pid )
{
    ...
    auto compare_bytes = []( const BYTE* src, const BYTE* cmp )
    {
        return *src == *cmp;
    };
    ...
    // If bytes are patched then we skip it.
    if ( compare_bytes( read_bytes[ 0 ], nop ) && compare_bytes( read_bytes[ 1 ], nop ) )
    {
        g_enabled = true;
        return 1;
    }
...
extern "C" __declspec(dllexport) int on_read_log( const char* msg, const char* zone )
{
    ...
    // 帝国南方堡                                                                                                                天幕魔导城
    if ( _zone.find( xorstr_( "\xE5\xB8\x9D\xE5\x9B\xBD\xE5\x8D\x97\xE6\x96\xB9\xE5\xA0\xA1" ) ) != std::string_view::npos || _zone.find( xorstr_( "\xE5\xA4\xA9\xE5\xB9\x95\xE9\xAD\x94\xE5\xAF\xBC\xE5\x9F\x8E" ) ) != std::string_view::npos )
    {
        // 00:0039:离开了休
        if ( _msg.find( xorstr_( "00:0039:\xE7\xA6\xBB\xE5\xBC\x80\xE4\xBA\x86\xE4\xBC\x91" ) ) != std::string_view::npos && !g_enabled )
        {
            g_enabled = true;
        ...
        // 00:0839:队伍
        if ( _msg.find( xorstr_( "00:0839:\xE9\x98\x9F\xE4\xBC\x8D" ) ) != std::string_view::npos && g_enabled )
        {
            g_enabled = false;
            // on_read_log can't be called without the plugin being fully initialized
            // so this is safe to call without a if-statement
